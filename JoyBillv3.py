...
class NeuralNetwork:
    ...
    def set(self, alpha=0.01, batch_size=32):
        self.alpha = alpha
        self.batch_size = batch_size

    def safety(self, dropBy:float, reduceLRBy:float):
        #SAFETY FEATURE
        ...
        for epoch in currentEpoch:
            for batch in range(self.n_batch):
                ...
                if batch == self.n_batch - 1:
                    ...
                    if self.auto_adjust:
                        #SAFETY FEATURE
                        while accuracy < self.train_accuracy - self.dropBy:
                    ...
                    currentEpoch.set_description(f'Epoch {self.all_Epochs - self.epochs + epoch + 1}/{self.all_Epochs}, accuracy={accuracy*100:.4f}%')
    ...
    def describe(self):
        ...
        if self.auto_adjust:
            print(f'Auto fixed x{self.inDanger}, final learning rate {self.alpha:.3f}')